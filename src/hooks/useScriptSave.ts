...
export const useScriptSave = (scriptId: string) => {
  ...
  const saveScript = useCallback(async (elements: ScriptElementType[], title?: string): Promise<boolean> => {
    ...
    try {
      ...
      const metadataSuccess = await saveScriptMetadata(validScriptId, userId, scriptTitle);
  ...
  const autoSave = useCallback(async (elements: ScriptElementType[], title?: string): Promise<boolean> => {
    if (saveInProgressRef.current) {
    ...
    try {
      ...
      const success = await saveScript(elements, title);
      if (success) {
